...
class GoogleAnalyticsDataApiBaseStream(GoogleAnalyticsDataApiAbstractStream):
    ...
    def get_json_schema(self) -> Mapping[str, Any]:
        ...
        schema["properties"]["isDataGolden"] = {
            "type": "boolean",
            "description": "Indicates if response to this request is golden or not."
    ...
    def parse_response(self, response: requests.Response, *, stream_state: Mapping[str, Any], stream_slice: Mapping[str, Any] = None,
        ...
        def _metric_type_to_python(metric_data: Tuple[str, str]) -> Any:
            ...
            python_type = metrics_type_to_python(metrics_type_map[metric_name])
            return metric_name, python_type(metric_value)

        for row in r.get("rows", []):
            data = dict(zip(dimensions, [v["value"] for v in row["dimensionValues"]]))
            data.update(dict(map(_metric_type_to_python, zip(metrics, [v["value"] for v in row["metricValues"]]))))
            rows.append(data)
        r["records"] = rows
